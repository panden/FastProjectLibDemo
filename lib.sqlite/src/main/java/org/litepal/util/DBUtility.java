...
public class DBUtility {
	...
	public static List<String> findAllTableNames(SQLiteDatabase db) {
		...
		try {
			...
			e.printStackTrace();
			throw new org.litepal.exceptions.DatabaseGenerateException(e.getMessage());
		} finally {
			if (cursor != null) {
				cursor.close();
			}
	 ...
	 * @throws org.litepal.exceptions.DatabaseGenerateException
	 */
	public static org.litepal.tablemanager.model.TableModel findPragmaTableInfo(String tableName, SQLiteDatabase db) {
		if (isTableExists(tableName, db)) {
            List<String> uniqueColumns = findUniqueColumns(tableName, db);
			org.litepal.tablemanager.model.TableModel tableModelDB = new org.litepal.tablemanager.model.TableModel();
			...
			throw new org.litepal.exceptions.DatabaseGenerateException(
					org.litepal.exceptions.DatabaseGenerateException.TABLE_DOES_NOT_EXIST_WHEN_EXECUTING + tableName);
		}
	}
