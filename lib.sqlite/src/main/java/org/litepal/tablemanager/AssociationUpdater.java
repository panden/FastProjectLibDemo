...
public abstract class AssociationUpdater extends Creator {
	...
	protected List<String> getForeignKeyColumns(TableModel tableModel) {
        ...
        List<org.litepal.tablemanager.model.ColumnModel> columnModelList = getTableModelFromDB(tableModel.getTableName()).getColumnModels();
		for (org.litepal.tablemanager.model.ColumnModel columnModel : columnModelList) {
			...
			if (isForeignKeyColumnFormat(columnModel.getColumnName())) {
                if (!tableModel.containsColumn(columnName)) {
                    ...
                    org.litepal.util.LogUtil.d(TAG, "getForeignKeyColumnNames >> foreign key column is " + columnName);
	...
	protected boolean isForeignKeyColumn(TableModel tableModel, String columnName) {
		return org.litepal.util.BaseUtility.containsIgnoreCases(getForeignKeyColumns(tableModel), columnName);
	}
	...
	protected void clearCopyInTableSchema(List<String> tableNames) {
		if (tableNames != null && !tableNames.isEmpty()) {
			...
			org.litepal.util.LogUtil.d(TAG, "clear table schema value sql is " + deleteData);
	...
	private List<String> findForeignKeyToRemove(TableModel tableModel) {
		...
		org.litepal.util.LogUtil.d(TAG, "findForeignKeyToRemove >> " + tableModel.getTableName() + " "
				+ removeRelations);
	...
	private List<String> findIntermediateTablesToDrop() {
		...
		org.litepal.util.LogUtil.d(TAG, "findIntermediateTablesToDrop >> " + intermediateTables);
	...
	protected String generateDataMigrationSQL(TableModel tableModel) {
		...
		List<org.litepal.tablemanager.model.ColumnModel> columnModels = tableModel.getColumnModels();
		if (!columnModels.isEmpty()) {
			StringBuilder sql = new StringBuilder();
			sql.append("insert into ").append(tableName).append("(");
			boolean needComma = false;
			for (org.litepal.tablemanager.model.ColumnModel columnModel : columnModels) {
				if (needComma) {
					sql.append(", ");
				}
				needComma = true;
	...
	private List<String> getRemoveColumnSQLs(Collection<String> removeColumnNames, String tableName) {
		...
		org.litepal.util.LogUtil.d(TAG, "generateRemoveColumnSQL >> " + alterToTempTableSQL);
		String createNewTableSQL = generateCreateNewTableSQL(removeColumnNames, tableModelFromDB);
		org.litepal.util.LogUtil.d(TAG, "generateRemoveColumnSQL >> " + createNewTableSQL);
		String dataMigrationSQL = generateDataMigrationSQL(tableModelFromDB);
		org.litepal.util.LogUtil.d(TAG, "generateRemoveColumnSQL >> " + dataMigrationSQL);
		String dropTempTableSQL = generateDropTempTableSQL(tableName);
		org.litepal.util.LogUtil.d(TAG, "generateRemoveColumnSQL >> " + dropTempTableSQL);
        List<String> sqls = new ArrayList<String>();
