...
public class ClusterQuery {
	...
	public synchronized <T> List<T> find(Class<T> modelClass, boolean isEager) {
		QueryHandler queryHandler = new QueryHandler(org.litepal.tablemanager.Connector.getDatabase());
		String limit;
		if (mOffset == null) {
			limit = mLimit;
		} else {
            ...
            public void run() {
                synchronized (DataSupport.class) {
                    ...
                    if (executor.getListener() != null) {
                        org.litepal.LitePal.getHandler().post(new Runnable() {
                            @Override
                            public void run() {
     ...
     * Note that the associated models won't be loaded by default considering
     * the efficiency, but you can do that by using
     * {@link ClusterQuery#findFirst(Class, boolean)}.
     *
     * @param modelClass
    ...
    /**
     * It is mostly same as {@link ClusterQuery#findFirst(Class)} but an isEager
     * parameter. If set true the associated models will be loaded as well.
     * <br>
     * Note that isEager will only work for one deep level relation, considering the query efficiency.
     * You have to implement on your own if you need to load multiple deepness of relation at once.
     ...
     * Note that the associated models won't be loaded by default considering
     * the efficiency, but you can do that by using
     * {@link ClusterQuery#findLast(Class, boolean)}.
     *
     * @param modelClass
     *            Which table to query and the object type to return.
     * @return An object with founded data from database, or null.
    ...
    /**
     * It is mostly same as {@link ClusterQuery#findLast(Class)} but an isEager
     * parameter. If set true the associated models will be loaded as well.
     * <br>
     * Note that isEager will only work for one deep level relation, considering the query efficiency.
     * You have to implement on your own if you need to load multiple deepness of relation at once.
	...
	public synchronized int count(Class<?> modelClass) {
		return count(org.litepal.util.BaseUtility.changeCase(modelClass.getSimpleName()));
	}
    ...
    public CountExecutor countAsync(Class<?> modelClass) {
        return countAsync(org.litepal.util.BaseUtility.changeCase(DBUtility.getTableNameByClassName(modelClass.getName())));
    }
	...
	public synchronized double average(Class<?> modelClass, String column) {
		return average(org.litepal.util.BaseUtility.changeCase(modelClass.getSimpleName()), column);
	}
    ...
    public AverageExecutor averageAsync(final Class<?> modelClass, final String column) {
        return averageAsync(org.litepal.util.BaseUtility.changeCase(DBUtility.getTableNameByClassName(modelClass.getName())), column);
    }
	...
	public synchronized <T> T max(Class<?> modelClass, String columnName, Class<T> columnType) {
		return max(org.litepal.util.BaseUtility.changeCase(modelClass.getSimpleName()), columnName, columnType);
	}
    ...
    public <T> FindExecutor maxAsync(final Class<?> modelClass, final String columnName, final Class<T> columnType) {
        return maxAsync(org.litepal.util.BaseUtility.changeCase(DBUtility.getTableNameByClassName(modelClass.getName())), columnName, columnType);
    }
	...
	public synchronized <T> T min(Class<?> modelClass, String columnName, Class<T> columnType) {
		return min(org.litepal.util.BaseUtility.changeCase(modelClass.getSimpleName()), columnName, columnType);
	}
    ...
    public <T> FindExecutor minAsync(final Class<?> modelClass, final String columnName, final Class<T> columnType) {
        return minAsync(org.litepal.util.BaseUtility.changeCase(DBUtility.getTableNameByClassName(modelClass.getName())), columnName, columnType);
    }
	...
	public synchronized <T> T sum(Class<?> modelClass, String columnName, Class<T> columnType) {
		return sum(org.litepal.util.BaseUtility.changeCase(modelClass.getSimpleName()), columnName, columnType);
	}
    ...
    public <T> FindExecutor sumAsync(final Class<?> modelClass, final String columnName, final Class<T> columnType) {
        return sumAsync(org.litepal.util.BaseUtility.changeCase(DBUtility.getTableNameByClassName(modelClass.getName())), columnName, columnType);
    }
