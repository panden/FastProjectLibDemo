...
public class Connector {
	...
	public synchronized static SQLiteDatabase getWritableDatabase() {
		...
		return litePalHelper.getWritableDatabase(sDBPassword);
	}

	/**
	 * Deprecated. Using {@link org.litepal.LitePal#getDatabase()} instead.
	 * 
	...
	public synchronized static SQLiteDatabase getReadableDatabase() {
		LitePalOpenHelper litePalHelper = buildConnection();
		return litePalHelper.getReadableDatabase(sDBPassword);
	}
	...
	public static void clearLitePalOpenHelperInstance() {
        if (mLitePalHelper != null) {
            mLitePalHelper.getWritableDatabase(sDBPassword).close();
            mLitePalHelper = null;
        }
	}
