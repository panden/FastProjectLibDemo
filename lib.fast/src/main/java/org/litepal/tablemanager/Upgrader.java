...
public class Upgrader extends AssociationUpdater {
	 ...
	 */
	protected org.litepal.tablemanager.model.TableModel mTableModel;

    /**
     * Model class for table from database.
     */
    protected org.litepal.tablemanager.model.TableModel mTableModelDB;
	...
	protected void createOrUpgradeTable(SQLiteDatabase db, boolean force) {
		mDb = db;
		for (org.litepal.tablemanager.model.TableModel tableModel : getAllTableModels()) {
			mTableModel = tableModel;
            mTableModelDB = getTableModelFromDB(tableModel.getTableName());
            org.litepal.util.LogUtil.d(TAG, "createOrUpgradeTable: model is " + mTableModel.getTableName());
            upgradeTable();
		}
	}
	...
	private void upgradeTable() {
        if (hasNewUniqueOrNotNullColumn()) {
            ...
            for (AssociationsInfo info : associationsInfo) {
                if (info.getAssociationType() == org.litepal.util.Const.Model.MANY_TO_ONE
                        || info.getAssociationType() == org.litepal.util.Const.Model.ONE_TO_ONE) {
    ...
    private boolean hasNewUniqueOrNotNullColumn() {
        List<org.litepal.tablemanager.model.ColumnModel> columnModelList = mTableModel.getColumnModels();
        for (org.litepal.tablemanager.model.ColumnModel columnModel : columnModelList) {
            org.litepal.tablemanager.model.ColumnModel columnModelDB = mTableModelDB.getColumnModelByName(columnModel.getColumnName());
	 ...
	 */
	private List<org.litepal.tablemanager.model.ColumnModel> findColumnsToAdd() {
        List<org.litepal.tablemanager.model.ColumnModel> columnsToAdd = new ArrayList<org.litepal.tablemanager.model.ColumnModel>();
        for (org.litepal.tablemanager.model.ColumnModel columnModel : mTableModel.getColumnModels()) {
	...
	private List<String> findColumnsToRemove() {
        ...
        List<org.litepal.tablemanager.model.ColumnModel> columnModelList = mTableModelDB.getColumnModels();
        for (org.litepal.tablemanager.model.ColumnModel columnModel : columnModelList) {
        ...
        org.litepal.util.LogUtil.d(TAG, "remove columns from " + tableName + " >> " + removeColumns);
	 ...
	 */
	private List<org.litepal.tablemanager.model.ColumnModel> findColumnTypesToChange() {
        List<org.litepal.tablemanager.model.ColumnModel> columnsToChangeType = new ArrayList<org.litepal.tablemanager.model.ColumnModel>();
        for (org.litepal.tablemanager.model.ColumnModel columnModelDB : mTableModelDB.getColumnModels()) {
            for (org.litepal.tablemanager.model.ColumnModel columnModel : mTableModel.getColumnModels()) {
                if (columnModelDB.getColumnName().equalsIgnoreCase(columnModel.getColumnName())) {
                    ...
                    if (!hasConstraintChanged) {
                        ...
                        org.litepal.util.LogUtil.d(TAG, "default value db is:" + columnModelDB.getDefaultValue() + ", default value is:" + columnModel.getDefaultValue());
	 ...
	 */
	private String generateAddColumnSQL(org.litepal.tablemanager.model.ColumnModel columnModel) {
	 ...
	 */
	private List<String> getAddColumnSQLs(List<org.litepal.tablemanager.model.ColumnModel> columnModelList) {
    ...
    private void removeColumns(List<String> removeColumnNames) {
        org.litepal.util.LogUtil.d(TAG, "do removeColumns " + removeColumnNames);
	 ...
	 *            List with ColumnModel to add new column.
	 */
	private void addColumns(List<org.litepal.tablemanager.model.ColumnModel> columnModelList) {
        org.litepal.util.LogUtil.d(TAG, "do addColumn");
	 ...
	 */
	private void changeColumnsType(List<org.litepal.tablemanager.model.ColumnModel> columnModelList) {
        org.litepal.util.LogUtil.d(TAG, "do changeColumnsType");
    ...
    private void changeColumnsConstraints() {
        if (hasConstraintChanged) {
            org.litepal.util.LogUtil.d(TAG, "do changeColumnsConstraints");
    ...
    private List<String> getChangeColumnsConstraintsSQL() {
        ...
        org.litepal.util.LogUtil.d(TAG, "generateChangeConstraintSQL >> ");
        for (String sql : sqls) {
            org.litepal.util.LogUtil.d(TAG, sql);
        }
        org.litepal.util.LogUtil.d(TAG, "<< generateChangeConstraintSQL");
        return sqls;
    }
    ...
    private List<String> generateAddForeignKeySQL() {
        ...
        for (String foreignKeyColumn : foreignKeyColumns) {
            if (!mTableModel.containsColumn(foreignKeyColumn)) {
                org.litepal.tablemanager.model.ColumnModel columnModel = new org.litepal.tablemanager.model.ColumnModel();
