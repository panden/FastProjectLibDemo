 ...
 * @since 1.0
 */
public abstract class Generator extends org.litepal.LitePalBase {
	 ...
	 * time.
	 */
	private Collection<org.litepal.tablemanager.model.TableModel> mTableModels;
	 ...
	 */
	protected Collection<org.litepal.tablemanager.model.TableModel> getAllTableModels() {
		if (mTableModels == null) {
			mTableModels = new ArrayList<org.litepal.tablemanager.model.TableModel>();
		}
		if (!canUseCache()) {
			mTableModels.clear();
			for (String className : org.litepal.parser.LitePalAttr.getInstance().getClassNames()) {
	...
	protected Collection<AssociationsModel> getAllAssociations() {
		if (mAllRelationModels == null || mAllRelationModels.isEmpty()) {
			mAllRelationModels = getAssociations(org.litepal.parser.LitePalAttr.getInstance().getClassNames());
		}
			...
			if (sqls != null && !sqls.isEmpty()) {
				for (String sql : sqls) {
                    if (!TextUtils.isEmpty(sql)) {
                        throwSQL = org.litepal.util.BaseUtility.changeCase(sql);
                        db.execSQL(throwSQL);
                    }
				}
			}
		} catch (SQLException e) {
			throw new org.litepal.exceptions.DatabaseGenerateException(org.litepal.exceptions.DatabaseGenerateException.SQL_ERROR + throwSQL);
		}
	}
	...
	private boolean canUseCache() {
		...
		return mTableModels.size() == org.litepal.parser.LitePalAttr.getInstance().getClassNames().size();
	}
